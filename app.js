...
app.delete(
  ...
  async (request, response) => {
    ...
    const checkValidTweet = `
      SELECT tweet FROM tweet.tweet INNER JOIN follower ON follower.following_user_id = tweet.user_id WHERE tweet.tweet_id = ${tweetId};
    `

    if (checkValidTweet === undefined) {
      response.status(401)
      response.send('Invalid Request')
    } else {
